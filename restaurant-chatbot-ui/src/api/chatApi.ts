...
//  Get or generate a persistent device ID for tracking the user's session
...
const API_URL = 'https://altschool-restaurant-chatbot-7n38.onrender.com';
...
export const sendMessage = async (message: string): Promise<ChatResponse> => {
  const { data } = await chatApi.post<ChatResponse>('/chat/message', { message });
  return data;
};